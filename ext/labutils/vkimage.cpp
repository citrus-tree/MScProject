...
namespace labutils
{
	...
	Image image_from_data_texture2d(std::vector<unsigned char> data, int aWidth, int aHeight, int bpp, VulkanContext const& aContext, VkCommandPool aCmdPool, Allocator const& aAllocator, VkFormat format)
	{
		const uint32_t uwidth = static_cast<uint32_t>(aWidth);
		const uint32_t uheight = static_cast<uint32_t>(aHeight);
		const uint32_t ubytes = uwidth * uheight * static_cast<uint32_t>(bpp);
		...
		memcpy(stagingPtr, data.data(), ubytes);
		vmaUnmapMemory(*aAllocator, stagingBuff.allocation);
